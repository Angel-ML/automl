...
private[feature] trait LassoSelectorParams extends Params
  ...
  /**
    * Number of features that selector will select, ordered by descending lasso coefficients. If the
...
class LassoSelector(override val uid: String)
  ...
  override def fit(dataset: Dataset[_]): LassoSelectorModel = {
    ...
    val sortedIndices: Array[Int] = argsort.argsortDenseVector_Double(BDV(coefficients))
      .toArray.reverse
...
class LassoSelectorModel(override val uid: String,
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    topKFeatures = selectedFeatures.take($(numTopFeatures)).sorted
    println(s"top-${topKFeatures.length} features = ${topKFeatures.mkString(",")}")
    ...
    val select = udf { vector: Vector =>
      vector match {
        ...
        case dv: DenseVector =>
          ...
          val values = topKFeatures.map(denseValues)
  ...
  override def copy(extra: ParamMap): LassoSelectorModel = {
    val copied = new LassoSelectorModel(uid, selectedFeatures)
